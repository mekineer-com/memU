...
| `app/graph.py` | `GraphMixin` — bounded graph reads (`graph_memory`, `graph_nodes`, `graph_related`), memory-summary edits (`update_memory_summary` — embed-first, whitespace-no-op guarded, writes `memory_item_edit_history`), and category-summary edits (`update_category_summary` — delegates to `category_summary_journal.py` helper); semantic edge predicates: `caused_by`, `evokes`, `conflicts_with`, `parallels`, `shaped_by`; `list_items_by_ids` backed by `memory_item.py` repo |
