...
| `app/memorize_categories.py` | Category seam for memorize: homeless-entry clustering, dynamic-category planning/creation, category init/scope mapping, and category-summary update rendering; agent-name fallback is "the soul" (not "the assistant"); dynamic-category planning LLM calls route through `_select_chat_client` (Claude-Code-aware, ensures claude_code=true uses step seam) |
