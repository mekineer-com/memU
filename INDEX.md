...
| `app/graph.py` | `GraphMixin` — bounded graph reads (`graph_memory`, `graph_nodes`, `graph_related`), Atomic search (`graph_search`: scoped FTS + vector over memory items, optional Python `since_days` cutoff), memory-summary edits (`update_memory_summary` — embed-first, whitespace-no-op guarded, writes `memory_item_edit_history`), category-summary edits (`update_category_summary` — delegates to `category_summary_journal.py` helper), Atomic approval review (`graph_list_pending`, approve helpers), and scoped hard-delete for memory items; semantic edge predicates: `caused_by`, `evokes`, `conflicts_with`, `parallels`, `shaped_by`; `list_items_by_ids` backed by `memory_item.py` repo |
