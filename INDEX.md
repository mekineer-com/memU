...
| `app/retrieve.py` | Retrieve workflow: rewrite query → embed → rank → judge; `_split_context_queries()` splits context so route step receives full chat-history context (`history_from_chat_x`, 2-anchor) while downstream sufficiency steps receive trimmed context (`history_from_recent_chat_x`, 1-anchor); `identity_context` is preserved across all steps and rendered as plain text at top of soul context |
