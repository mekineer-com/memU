...
| `diary/` | `self_model_update.py` | Diary generation & self-model reflection. `self_model_update.py` includes `<life_goals>` XML section (add/remove; max 3 active; most sessions leave empty) and `<soul_observations>` with optional `<supersedes><id>...</id></supersedes>` per observation (IDs from retrieved background memories; scope-validated at write time). |
